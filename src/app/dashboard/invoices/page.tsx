...
export default function InvoicesPage() {
  ...
  function onView(r: InvoiceListItem) {
    router.push(`/dashboard/invoices/${r.id}/view`);
  }
  ...
  async function onDownload(r: InvoiceListItem) {
    try {
      ...
      const taxTotal =
        inv?.tax_total && Number(inv.tax_total) > 0
          ? Number(inv.tax_total)
          : mappedItems.reduce((sum, item) => sum + item.tax, 0);

      let grandTotal =
        inv?.grand_total && Number(inv.grand_total) > 0
      ...
      const taxRate =
        mappedItems.length > 0 && mappedItems[0].tax > 0
          ? Math.round((mappedItems[0].tax / (mappedItems[0].price * mappedItems[0].qty)) * 100)
          : 10;
        ...
        <Card className="shadow-sm">
          <CardHeader className="space-y-3 pb-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <CardTitle className="text-xl md:text-2xl whitespace-nowrap shrink-0">All Invoices</CardTitle>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      filtered.map((row, idx) => {
                        ...
                        const due = safeDate(
                          (row as any)?.valid_until || (row as any)?.due,
                        );
                                ...
                                >
                                  ...
                                  {canUpdate && (
                                    <DropdownMenuItem
                                      onClick={() =>
                                        router.push(
                                          `/dashboard/invoices/${row.id}/edit`,
                                        )
                                      }
                                      className="cursor-pointer"
                                    >
